...
// WorkerUsage tracks AI Worker API usage statistics
...
func (*WorkerUsage) Table() string { return "worker_usage" }


// GetOrCreateUsage returns the usage stats, creating if necessary
func GetOrCreateUsage() (*WorkerUsage, error) {
	// Try to get existing usage record (there should only be one)
	usages, err := WorkerUsages.Search("ORDER BY ID LIMIT 1")
	if err != nil {
		return nil, err
	}
	...
	if len(usages) > 0 {
		...
		if usage.MonthReset.Before(time.Date(now.Year(), now.Month(), 1, 0, 0, 0, 0, time.UTC)) {
			...
			if err := WorkerUsages.Update(usage); err != nil {
				return nil, err
			}
		}
		return usage, nil
	...
	return WorkerUsages.Insert(usage)
}
...
func UpdateUsage(inputTokens, outputTokens int) error {
	...
	return WorkerUsages.Update(usage)
}

// GetUsageStats returns the current usage statistics
func GetUsageStats() (*WorkerUsage, error) {
