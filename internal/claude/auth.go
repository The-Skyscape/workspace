...
// AuthManager handles Claude API authentication
...
func (am *AuthManager) Initialize() error {
	...
	if am.vault == nil {
		log.Println("Claude: Vault not configured, skipping initialization")
		return nil
	}

	secret, err := am.vault.GetSecret("integrations/claude")
	if err != nil {
		// Not an error if not configured yet
		log.Println("Claude: No API key configured")
		return nil
	}
...
func (am *AuthManager) SetAPIKey(apiKey string) error {
	...
	if err := am.vault.StoreSecret("integrations/claude", data); err != nil {
		return fmt.Errorf("failed to store API key: %w", err)
	}

	am.apiKey = apiKey
	am.configured = true
	log.Println("Claude: API key stored in vault")
...
// IsConfigured checks if Claude is configured
...
// RemoveConfiguration removes the Claude configuration
func (am *AuthManager) RemoveConfiguration() error {
	...
	if err := am.vault.DeleteSecret("integrations/claude"); err != nil {
		return fmt.Errorf("failed to remove configuration: %w", err)
	}

	am.apiKey = ""
	am.configured = false
	log.Println("Claude: Configuration removed")
